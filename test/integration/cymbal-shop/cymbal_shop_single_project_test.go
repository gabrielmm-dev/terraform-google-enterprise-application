...
func TestSingleProjectSourceCymbalShop(t *testing.T) {
	...
	projectID := gitLab.GetTFSetupJsonOutput("harness_project_ids").Get("cymbal-shop").String()
	...
	t.Run(servicePath, func(t *testing.T) {
		...
		appsource.DefineVerify(func(assert *assert.Assertions) {
			...
			// Push cymbal shop app source code
