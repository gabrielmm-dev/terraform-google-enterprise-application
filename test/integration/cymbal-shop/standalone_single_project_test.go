...
func TestStandaloneSingleProjectCymbalShop(t *testing.T) {
	...
	projectID := setupOutput.GetJsonOutput("harness_project_ids").Get("cymbal-shop").String()
	...
	ncc_config := map[string]interface{}{
		...
		"spoke_name":        "vpc-spoke-cymbal-shop",
		"spoke_description": "Spoke for cymbal shop single project example",
	}
	vars := map[string]interface{}{
		...
		"logging_bucket":         loggingBucket.GetJsonOutput("logging_bucket").Get("cymbal-shop").String(),
		"bucket_kms_key":         loggingBucket.GetJsonOutput("bucket_kms_key").Get("cymbal-shop").String(),
		"attestation_kms_key":    loggingBucket.GetJsonOutput("attestation_kms_key").Get("cymbal-shop").String(),
	...
	standaloneSingleProjT := tft.NewTFBlueprintTest(t,
		tft.WithVars(vars),
		tft.WithTFDir("../../../examples/cymbal-shop/standalone-single-project"),
	...
	standaloneSingleProjT.DefineVerify(func(assert *assert.Assertions) {
		...
		// Cloud Armor
		cloudArmorName := "cs-eab-cloud-armor"
